...
in sequence, routing work to either OpenRouter (via OpenClaw HTTP API on
port 9100) or the Claude CLI executor depending on each block's config.

IMPORTANT: All OpenRouter/LLM work is delegated to OpenClaw via HTTP.
This ensures clean separation -- no Agent Zero tool definitions or system
prompts leak into task execution.
"""
...
def _execute_openrouter(prompt: str, model: str, system_prompt: str = "") -> dict[str, Any]:
    """Execute a task through OpenRouter via OpenClaw HTTP API.

    This dispatches the job to OpenClaw on port 9100 via HTTP, then polls
    for completion. OpenClaw's LangGraph runtime calls OpenRouter with
    ONLY the task prompt and agent system prompt -- no Agent Zero framework
    overhead leaks into the execution.
    """
    start = time.monotonic()

    # Build the agent dict for the HTTP payload
    agent_payload: dict[str, Any] = {"role": "general", "model": model}
    if system_prompt:
        agent_payload["_system_prompt_override"] = system_prompt

    # Dispatch job via HTTP to OpenClaw (port 9100)
    ...
    if not dispatch_result.get("ok"):
        ...
        return {
            "success": False,
            "output": "",
            "error": dispatch_result.get("error", "dispatch_failed"),
            "executor": "OpenRouter (via OpenClaw HTTP)",
            "duration_seconds": round(elapsed, 3),
            "failure_type": None,
        }

    job_id = dispatch_result["job_id"]
    _log("openclaw_job_dispatched", "pipeline", job_id, f"model={model}", layer="openclaw", model=model)

    # Poll for completion
    poll_interval = int(os.getenv("OPENCLAW_PIPELINE_POLL_SECONDS", "5"))
    timeout = int(os.getenv("OPENCLAW_PIPELINE_TIMEOUT_SECONDS", "300"))
    ...
    while time.monotonic() < deadline:
        ...
        status_result = get_status(job_id=job_id)
        ...
        if state == "completed":
            elapsed = time.monotonic() - start
            output = str(raw.get("result") or raw.get("output") or "")
            return {
                "success": True,
                "output": output,
                "executor": "OpenRouter (via OpenClaw HTTP)",
        ...
        if state == "failed":
            elapsed = time.monotonic() - start
            error_msg = str(raw.get("error") or "openclaw_job_failed")
            return {
                "success": False,
                "output": "",
                "executor": "OpenRouter (via OpenClaw HTTP)",
    ...
    return {
        ...
        "executor": "OpenRouter (via OpenClaw HTTP)",
        "duration_seconds": round(elapsed, 3),
        "error": f"timeout_after_{timeout}s (last_state={last_state})",
        "failure_type": None,
    }
